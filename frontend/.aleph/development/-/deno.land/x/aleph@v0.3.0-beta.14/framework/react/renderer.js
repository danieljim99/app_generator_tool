// https://deno.land/x/aleph@v0.3.0-beta.14/framework/react/renderer.ts
...
async function render(url, App, nestedPageComponents, styles) {
  const global = window;
  const ret = {
    head: [],
    body: "",
  ...
  nestedPageComponents.forEach(({ specifier, props }) => {
    if (util.isPlainObject(props)) {
      ...
      if (typeof props.$revalidate === "number" && props.$revalidate > 0) {
        expires = Date.now() + Math.round(props.$revalidate * 1e3);
      }
